...
def TableToTableAllocation(
    ...
    recipient_capacity_fields,
    recipient_suitability_fields,
    output_folder,
    allocation_name,
    recipient_mix_fields=None,
    consumption_weights=None,
    ...
    single_suit, recipient_suitability_fields = _adaptParameters(
        recipient_suitability_fields
    )
    ...
    if (
        consumption_weights is None
    ):  # and single_ctrl = False and single_cap = True: ????
    ...
    if link_is_string:
        alloc_dt_list = [
            (recipient_link_field, "|S{}".format(link_is_string[0].length))
        ]
    else:
        alloc_dt_list = [(recipient_link_field, "<f8")]
    if id_is_string:
        alloc_dt_list.append(
            (recipient_id_field, "|S{}".format(id_is_string[0].length))
        )
    else:
    ...
    control_rows = ap.da.TableToNumPyArray(
        control_table, control_fields, control_where_clause, skip_nulls=True
    )
    for control_row in control_rows:
        control_id = control_row[control_id_field]
        control_totals_dict = dict(
            list(
                zip(
                    control_total_fields, [control_row[i] for i in control_total_fields]
                )
            )
        )
        ...
        recipient_rows = ap.da.TableToNumPyArray(
            recipient_table, recipient_fields, expr, True
        )
        recipient_rows = np.array(
            recipient_rows,
            dtype=np.dtype(
                [
                    (cname, ctype) if ctype != "<i4" else (cname, "<f8")
                    for cname, ctype in recipient_rows.dtype.descr
                ]
            ),
        )

        # allocate activities
        this_alloc, this_unalloc = _allocateValues(
            control_id,
            control_totals_dict,
            control_total_fields,
            recipient_rows,
    ...
    ap.da.NumPyArrayToTable(
        alloc_array, "{}\\{}_alloc.dbf".format(output_folder, allocation_name)
    )
    ap.da.NumPyArrayToTable(
        unalloc_array, "{}\\{}_unalloc.dbf".format(output_folder, allocation_name)
    )
...
def _allocateValues(
    ...
    total_cap = sum([sum(recipient_rows[field]) for field in recipient_capacity_fields])
    total_suit = sum(
        [sum(recipient_rows[field]) for field in recipient_suitability_fields]
    )
    count = -1
    chex = 0

    while (
        sum(control_totals.values()) > 0
    ):  # loop until all control totals have been allocated
        count += 1
        if total_suit == 0 or total_cap < 1:
            # no suitable recipients or no remaining capacity
            print(
                "suitability or capacity exhausted for control area ({})".format(
                    control_id
                )
            )
            unallocated = _makeUnallocatedRow(
                control_id, unallocated_dict, control_total_fields
            )
            allocated = _makeAllocatedRows(
                control_id, allocated_dict, control_total_fields
            )
        ...
        if single_ctrl:
            ...
            if single_suit:
                ...
                activity_choice_array = np.array(
                    list(
                        zip(list(control_totals.keys()), list(control_totals.values()))
                    ),
                    dtype=np.dtype([("activity", "|S255"), ("control_value", "<f8")]),
                )
                activity_row = _selectRandomRowFromArray(
                    activity_choice_array, ["control_value"]
                )
        ...
        if single_ctrl or single_suit:
            ...
            valid_cap_rows = _fetchValidRecipients(
                recipient_rows,
                [
                    recipient_capacity_fields[i]
                    for i in range(len(control_total_fields))
                    if control_total_fields[i] in list(control_totals.keys())
                ],
                1,
                True,
            )
            ...
            valid_cap_rows = _fetchValidRecipients(
                recipient_rows, recipient_capacity_fields, 1, True
            )
        else:
            # fetch recipient rows that have capacity for the activity being allocated
            valid_cap_rows = _fetchValidRecipients(
                recipient_rows, [recipient_capacity_fields[allocation_idx]], 1, True
            )
        ...
        allocation_row_idx = np.where(
            recipient_rows[recipient_id_field] == recipient_id
        )[0][0]
        # if activity has not been determined yet, randomly choose activity now
        if activity == "_unknown":
            mix_indices = [
                control_total_fields.index(field)
                for field in control_total_fields
                if field in list(control_totals.keys())
            ]
            dtype = np.dtype([("activity", "|S255"), ("mix_value", "<f8")])
            # capacity check
            if single_cap:
                search_rows = [
                    (control_total_fields[i], allocation_row[recipient_mix_fields[i]])
                ...
                if not search_rows:
                    ...
                    continue
            activity_row = _selectRandomRowFromArray(
                np.array(search_rows, dtype=dtype), ["mix_value"]
            )
            activity = activity_row["activity"]
            allocation_idx = control_total_fields.index(activity)
            ...
            if single_cap:
                ...
                search_rows = list(
                    zip(
                        recipient_capacity_fields,
                        [
                            allocation_row[field]
                            if allocation_row[field] >= 1
                            else 0  #######>=increment?
                            for field in recipient_capacity_fields
                        ],
                    )
                )
                reduction_field = _selectRandomRowFromArray(
                    np.array(
                        search_rows,
                        dtype=np.dtype([("cap_field", "|S255"), ("cap_calue", "<f8")]),
                    ),
                    ["cap_value"],
                )["cap_field"]
                recipient_rows[allocation_row_idx][
                    reduction_field
                ] -= 1  ########increment?
        else:
            if single_cap:
                ...
                diminish_qty = (
                    1 * consumption_weight
                )  #######increment * consumption weight?
                recipient_rows[allocation_row_idx][
                    recipient_capacity_fields[0]
                ] -= diminish_qty
            else:
                # reduce the capacity for the activity being allocated by the increment (1)
                recipient_rows[allocation_row_idx][
                    recipient_capacity_fields[allocation_idx]
                ] -= 1  ##### increment
                # reduce capacities for each activity by the diminish_qty (increment * consumption weight)
                for i in range(len(control_total_fields)):
                    if i != allocation_idx:
                        consumption_weight = consumption_weights[
                            allocation_idx
                        ] / float(consumption_weights[i])
                        diminish_qty = (
                            1 * consumption_weight
                        )  #######increment * consumption weight
                        recipient_rows[allocation_row_idx][
    ...
    if not single_ctrl:
        ...
        if not single_cap:
            ...
            if len(recipient_capacity_fields) != len(control_total_fields):
                raise ValueError(
                    "Input Error 0000: unequal number of fields defining control totals and recipient capacities"
                )
        else:
            ...
            if len(consumption_weights) != len(control_total_fields):
                raise ValueError(
                    "Input Error 0001: unequal number of fields defining activities to allocate and consumption weights"
                )
        ...
        if not single_suit:
            ...
            if len(recipient_suitability_fields) != len(control_total_fields):
                raise ValueError(
                    "Input Error 0002: unequal number of fields defining suitability scores and recipient control totals/capacities"
                )
        else:
            # mult. ctrls using single suit must have mix fields equal in number to ctrl fields
            if not type(recipient_mix_fields) is list or len(
                recipient_mix_fields
            ) != len(control_total_fields):
                raise ValueError(
                    "Input Error 0003: argument is not a list or unequal number of fields defining mix values and recipient control totals/capacities"
                )
...
def _makeAllocatedRows(control_id, allocated_dict, control_total_fields):
    ...
    for field in control_total_fields:
        print(
            "\t\t{}: {}".format(
                field, sum([sub_dict.get(field, 0) for sub_dict in sub_dicts])
            )
        )
    return [
        tuple(
            [str(control_id), str(recipient_id)]
            + [
                allocated_dict[recipient_id].get(field, 0)
                for field in control_total_fields
            ]
        )
...
def _selectRandomRowFromArray(array, score_fields):
    ...
    total_score = float(
        sum([np.sum(array[score_field]) for score_field in score_fields])
    )
    weights = [
        sum(row[score_field] for score_field in score_fields) / total_score
        for row in array
    ]
