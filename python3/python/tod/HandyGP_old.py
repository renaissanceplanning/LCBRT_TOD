...
def maximumOverlapSpatialJoin(
    ...
    in_layer = arcpy.MakeFeatureLayer_management(
        in_features, str(uuid.uuid1()), where_clause=in_expression
    )
    target_layer = arcpy.MakeFeatureLayer_management(
        target_features, str(uuid.uuid1()), where_clause=target_expression
    )
    ...
    intersect = arcpy.Intersect_analysis(
        in_layer, "in_memory\\MOSJ_{}".format(_cleanUUID(uuid.uuid1()))
    )
    ...
    arcpy.CalculateField_management(
        intersect, oa_field, "!shape.area@squarefeet!", "PYTHON"
    )
    ...
    dissolve = arcpy.Dissolve_management(
        intersect,
        "in_memory\\MOSJdis_{}".format(_cleanUUID(uuid.uuid1())),
        [in_id_field, target_id_field],
        "{} SUM".format(oa_field),
        "MULTI_PART",
    )
    ...
    overlap_dict = {}
    with arcpy.da.SearchCursor(
        dissolve, [in_id_field, target_id_field, "{} SUM".format(oa_field)]
    ) as c:
        for r in c:
            i, t, oa = r
    ...
    if output_type == "FIELD":
        ...
        with arcpy.da.UpdateCursor(
            in_features, [in_id_field, out_field_name, oa_field]
        ) as c:
            for r in c:
                i = r[0]
...
def multiRingBufferNoOverlap(
    ...
    for buffer_distance in buffer_distances:
        ...
        id_list = []
        with arcpy.da.SearchCursor(
            in_features, [id_field, "SHAPE@"], spatial_reference=sr
        ) as c:
            for r in c:
                in_id, in_point = r
            ...
            for other_id, other_buffer in zip(id_list, buffers):
                if not this_buffer.disjoint(other_buffer) and this_id != other_id:
                    ...
                    if intersect_points.partCount > 1:
                        ...
                        for cut in cuts:
                            this_dist = _getDistanceBetweenPoints(
                                cut.centroid, this_buffer.centroid, sr
                            )
                            other_dist = _getDistanceBetweenPoints(
                                cut.centroid, other_buffer.centroid, sr
                            )
