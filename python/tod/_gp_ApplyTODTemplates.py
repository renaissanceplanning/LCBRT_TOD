...
if __name__ == '__main__':
    ...
    fishnet_id = arcpy.GetParameterAsText(2) # field, from fishnet, auto search for CELL_ID
    fishnet_suitability_field = arcpy.GetParameterAsText(3) # field, from fishnet, numeric
    fishnet_where_clause = arcpy.GetParameterAsText(4) #expression, from fishnet, optional
    ...
    impedance_attribute = arcpy.GetParameterAsText(8) # string, from network_dataset, optional
    restrictions = arcpy.GetParameterAsText(9) # string, multivalue, form network_dataset, optional
    ...
    if network_dataset:
        if not impedance_attribute:
            raise ValueError("an impedance attribute must be specified when using a network dataset")
        else:
    ...
    TOD.applyTODTemplates(in_gdb, fishnet_fc, fishnet_id, technology_name,
                          fishnet_suitability_field=fishnet_suitability_field,
                          fishnet_where_clause=fishnet_where_clause,
                          sr=sr, network_dataset=network_dataset,
                          impedance_attribute=impedance_attribute,
                          restrictions=restrictions, preset_stations_field=preset_field,
                          weight_by_area=weight_by_area, share_threshold=share_threshold)
